...
<section class="cover"><small>SEOGEO · v1.0 WEBSITE READINESS REPORT</small><h1>${escapeHtml(audit.title)}</h1><p>${escapeHtml(audit.url)}</p><div class="meta"><span>분석일 ${escapeHtml(audit.lastScanned)}</span><span>보고서 생성 ${generatedAt}</span><span>점수 모델 ${escapeHtml(audit.scoreModelVersion ?? 'v1.0-r1')}</span></div></section>
<section class="section"><h2>핵심 요약</h2><div class="summary"><div class="hero-score"><span>SEO 기반 품질</span><b>${audit.seoFoundationScore ?? audit.overallScore}<small>/100</small></b></div><div class="hero-score ai"><span>AI 검색 가시성</span><b>${audit.aiCitationReadinessScore ?? audit.academicGeoScore}<small>/100</small></b></div><div class="hero-score"><span>네이버 노출 기반</span><b>${naverScore}</b></div><div class="summary-card"><span>세부 영역별 점수</span>${categoryHtml}</div></div><p class="note">SEOGEO 자체 준비도 지수이며 공식 검색 순위나 노출을 보장하지 않습니다. 측정 신뢰도: ${escapeHtml(audit.measurementConfidence ?? 'low')}.</p></section>
